// 11.38.cpp : 此文件包含 "main" 函数。程序执行将在此处开始并结束。
//

#include <bits/stdc++.h>
#include <unordered_map>
using namespace std;
void wordCounting()
{
	unordered_map<string, size_t> wordCount;
	string word;
	while (cin >> word)
	{
		++wordCount[word];
	}
	for (const auto& c : wordCount)
	{
		cout << c.first << " " << c.second << (c.second > 1 ? "times" : "time") << "\n";
	}
}
void wordTransformation()
{
	ifstream ifsMap("../../../data/word_transformation.txt");
	ifstream ifsContent("../../../data/given_to_transform.txt");
	if (!ifsMap || !ifsContent)
	{
		cerr << "Can't find the documents\n";
		return;
	}
	unordered_map<string, string> transMap;
	string key, value;
	while (ifsMap >> key && getline(ifsMap, value))
	{
		if (value.size() > 1)
		{
			transMap[key] = value.substr(1).substr(0, value.find_last_not_of(' '));
		}
	}
	string text, word;
	while (getline(ifsContent, text))
	{
		istringstream is(text);
		while(is >> word)
		{
			auto mapIt = transMap.find(word);
			cout << (mapIt == transMap.cend() ? word : mapIt->second) << " ";
		}
		cout << "\n";
	}
}
int main()
{
	wordTransformation();
}

// 运行程序: Ctrl + F5 或调试 >“开始执行(不调试)”菜单
// 调试程序: F5 或调试 >“开始调试”菜单

// 入门提示: 
//   1. 使用解决方案资源管理器窗口添加/管理文件
//   2. 使用团队资源管理器窗口连接到源代码管理
//   3. 使用输出窗口查看生成输出和其他消息
//   4. 使用错误列表窗口查看错误
//   5. 转到“项目”>“添加新项”以创建新的代码文件，或转到“项目”>“添加现有项”以将现有代码文件添加到项目
//   6. 将来，若要再次打开此项目，请转到“文件”>“打开”>“项目”并选择 .sln 文件
